#!/usr/bin/env python3
"""Regression tests for exhaustive Hetzner security client method generation."""
...
import generate_security_client_methods as generator
...
def main() -> int:
    operations = [
        ...
        if operation.service == "security"
    ]
    require(len(operations) == 14, "Security operation count changed")
    operation_ids = [operation.operation_id for operation in operations]
    require(operation_ids == sorted(operation_ids), "Security operations are not sorted")
    require(
        len(operation_ids) == len(set(operation_ids)),
        "Security operations are not unique",
    )
    ...
    require(
        permit_counts
        == {"none": 7, "mutation": 5, "destructive": 2, "cost": 0},
        "Security permit classes changed",
    )
    require(
        sum(operation.pagination == "yes" for operation in operations) == 4,
        "Security pagination count changed",
    )

    rendered = generator.render()
    generated_rows = rendered.rsplit("security_client_methods!(", 1)[1]
    require(generated_rows.count("    (") == 14, "generated row count changed")
    ...
    require(
        "SecurityService" in rendered,
        "generated execution lost Security service typing",
    )
    require(
    ...
    permitted = rendered.split("macro_rules! permitted_method", 1)[1].split(
        "macro_rules! security_client_method", 1
    )[0]
    require(
    ...
    print("14 Security client methods, policy classes, and cleanup paths tested.")
    return 0
