...
import shutil
from generate_cratesio_versions import PATHS, OUTPUT, render
...
def main():
    ...
    feature_guards()
    print("Version projection identities, fail-closed schema constraints and feature guards passed.")
...
def feature_guards():
    ...
    root = tests.fixture()
    try:
        module = root / tests.checker.CRATE / "src/versions/mod.rs"
        original = module.read_text(encoding="ascii")
        guards = [('#[cfg(any(feature = "blocking", feature = "async"))]\nmod client;', "mod client;", "versions client guard")]
        guards.extend((f'#[cfg(feature = "alloc")]\nmod {name};', f'mod {name};', "versions allocation guard")
