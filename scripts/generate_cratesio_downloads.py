#!/usr/bin/env python3
"""Check download schema projections and source-derived JSON fixtures."""
import argparse
import json
...
OUTPUT = ROOT / "crates/cloud-sdk-cratesio/src/downloads"
PATHS = {
    "get_crate_downloads": "/api/v1/crates/{name}/downloads",
    "get_version_downloads": "/api/v1/crates/{name}/{version}/downloads",
    "list_reverse_dependencies": "/api/v1/crates/{name}/reverse_dependencies",
    "download_version": "/api/v1/crates/{name}/{version}/download",
}
...
def render(document):
    ...
    for name in PATHS:
        ...
        if name == "list_reverse_dependencies":
            value["meta"] = {"total": 1}
    ...
    table = table.replace("generate_cratesio_catalog.py", "generate_cratesio_downloads.py")
...
def main():
    ...
    print("4 download JSON projections and fixtures passed.")
