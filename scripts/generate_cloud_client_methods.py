...
def render_service(
    *,
    ...
    if len(operations) != expected_operations:
        raise ValueError(f"{service_label} client operation count changed")
    header = HEADER
    if service != "cloud":
        replacements = (
            ("CloudService", service_marker),
            ("CloudReadResult", f"{type_prefix}ReadResult"),
            ("CloudClientMethodDescriptor", f"{type_prefix}ClientMethodDescriptor"),
            ("CLOUD_CLIENT_METHODS", f"{service_label.upper()}_CLIENT_METHODS"),
            ("cloud_client_methods", f"{service}_client_methods"),
            ("cloud_client_method", f"{service}_client_method"),
            ("Cloud client", f"{service_label} client"),
            ("Cloud operation", f"{service_label} operation"),
        )
        for old, new in replacements:
            header = header.replace(old, new)
    rows = "\n".join(render_row(operation) for operation in operations)
    return f"{header}#[rustfmt::skip]\n{service}_client_methods!(\n{rows}\n);\n"
...
def render() -> str:
    return render_service(
        ...
        service_label="Cloud",
        type_prefix="Cloud",
        service_marker="CloudService",
        expected_operations=EXPECTED_CLOUD_OPERATIONS,
    )
