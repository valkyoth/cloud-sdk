#!/usr/bin/env python3
"""Regression tests for exhaustive Hetzner DNS client method generation."""
...
import generate_dns_client_methods as generator
...
def main() -> int:
    operations = [
        ...
        if operation.service == "dns"
    ]
    require(len(operations) == 24, "DNS operation count changed")
    operation_ids = [operation.operation_id for operation in operations]
    require(operation_ids == sorted(operation_ids), "DNS operations are not sorted")
    require(len(operation_ids) == len(set(operation_ids)), "DNS operations are not unique")
    ...
    require(
        permit_counts
        == {"none": 8, "mutation": 9, "destructive": 7, "cost": 0},
        "DNS permit classes changed",
    )
    require(
        sum(operation.pagination == "yes" for operation in operations) == 4,
        "DNS pagination count changed",
    )

    rendered = generator.render()
    generated_rows = rendered.rsplit("dns_client_methods!(", 1)[1]
    require(generated_rows.count("    (") == 24, "generated row count changed")
    ...
    require("DnsService" in rendered, "generated execution lost DNS service typing")
    require(
    ...
    permitted = rendered.split("macro_rules! permitted_method", 1)[1].split(
        "macro_rules! dns_client_method", 1
    )[0]
    require(
    ...
    print("24 DNS client methods, policy classes, and cleanup paths tested.")
    return 0
