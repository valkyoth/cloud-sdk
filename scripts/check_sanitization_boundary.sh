...
if ! printf '%s\n' "$boundary_tree" | grep -Fq 'sanitization v2.1.0'; then
...
sanitization_tree=$(cargo tree -p sanitization@2.1.0 --edges normal)
...
for package in cloud-sdk cloud-sdk-hetzner; do
    ...
    if ! printf '%s\n' "$default_tree" |
        grep -Fq 'sanitization v2.1.0'; then
