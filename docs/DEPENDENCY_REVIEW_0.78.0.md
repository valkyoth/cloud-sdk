...
Default Robot request preparation reuses the existing provider-neutral
operation, transport, buffer, and sanitization boundaries. Owned server models
reuse the existing opt-in strict JSON parser and protected string storage under
`serde`. The committed source-contract checker uses only the Python standard
