...
- `std` paths are prohibited except under explicitly reviewed optional
  ...
  `blocking-rustls-webpki-roots`, and `async-rustls` features.
- FIPS remains outside active manifests and transport source until the Brynja
  admission conditions in `docs/FIPS_DEFERMENT.md` are satisfied.
...
- Provider identifiers use one package-name segment, such as `hetzner`,
  `scaleway`, `digitalocean`, or `ovhcloud`.
...
- Nested package names such as `cloud-sdk-ovhcloud-dns`,
