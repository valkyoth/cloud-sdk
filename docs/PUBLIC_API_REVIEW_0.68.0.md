...
and `response_identity()` accessors to `OperationDescriptor`. The sealed
...
policy evidence emitted from compiled Rust descriptors. Written endpoint paths
are validated against those descriptor templates before preparation succeeds.
...
are unchanged. A mismatched internal path encoder now fails closed.
