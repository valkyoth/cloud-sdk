...
The existing cross-thread traits receive provider-neutral blanket
implementations of the local contracts. Reqwest remains a dependency-only
patch and continues to use its reviewed Tokio boundary only when explicitly
enabled.
...
| `cloud-sdk-reqwest` | `0.31.1` | `0.31.2` | Dependency-only core update. |
...
- dropped-future body/header cleanup and possibly-sent classification;
