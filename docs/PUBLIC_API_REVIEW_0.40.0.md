...
and clears complete body/header storage when dropped without a commit. Direct
writer body/header mutation and commitment are private, so external transports
cannot bypass the attempt guard. Compile-fail coverage locks this boundary.
...
bodies and header values in cleanup-owning allocations. The request-copy cap is
an explicit first-party reqwest adapter guarantee rather than a requirement of
the provider-neutral raw executor traits.
...
parsed or sensitive data. The wire target reaches the 64 KiB encoded-field
boundary and limit-plus-one rejection with canonical parser-state seeds.
...
therefore rejected by `RequestHeader`, and direct `ResponseWriter` mutation
requires `ResponseWriter::begin_attempt`. Applications should continue
supplying the validated user agent through reqwest client builders.
