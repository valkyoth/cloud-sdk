...
Stop gate: `v1.0.0 full-project pentest and final retest passed. Run the final release gate for this exact commit before stable publication.`

Status: one Low direct-dependency pinning finding was remediated without changing
the resolved graph. The independent retest is fully green and found no other
issue. The final public release gate and green GitHub CI and CodeQL remain
required before signed tagging and crates.io publication.
