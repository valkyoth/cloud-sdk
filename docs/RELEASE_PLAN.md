...
Verification: all method/path/form combinations, target-specific response policies, date and canonical-address boundaries, identity mismatch, cross-operation compile failure, permit-authorized exact-request response decoding, mismatched mutation date/reason/reservation availability/reserved/active-state rejection, Unicode control and directional-format rejection, protected allocation-error classification, source fixture/checker regressions, checked-response fuzz seeds, alloc/Serde feature graphs, and `scripts/release_0_79_gate.sh`.
