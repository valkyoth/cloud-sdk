...
4. Complete higher-level Cargo publish/owner/yank/unyank/search workflows and
   ...
   explicit Cargo owner profile described below without weakening website models.
...
Owner-list compatibility still needs a distinct reviewed profile:
...
- Do not weaken the website decoder or silently forward credentials to all
  anonymous account operations. Add a separately reviewed Cargo owner profile,
  with explicit authentication admission and its own bounded decoder/tests.

The characterization test preserves this distinction until that profile exists.
