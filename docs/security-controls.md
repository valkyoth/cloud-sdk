...
| transport contract | shared-reference blocking plus executor-neutral async traits; normalized credential-free endpoint identity; exact Hetzner official-endpoint verifier for both v1 API families; origin-form targets reject leading `//`, backslash, fragments, controls, spaces, and non-ASCII; responses borrow their initialized caller-buffer slice; no independent untrusted body length; no authentication, headers, TLS, retry, runtime, or network implementation |
