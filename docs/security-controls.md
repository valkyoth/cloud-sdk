...
| live smoke harness | ignored by default; exact read-only opt-in; fixed official origin; private regular token-file input; bounded reads; Unix symlink, permission, and opened-file identity checks; typed GET-only catalog probes; static diagnostics without token paths, bodies, or IDs; destructive mode rejected |
