...
| pagination state | no_std caller-driven cursor with a hard page limit; exact page transitions and known-last terminal state; decoded entries bounded by `per_page` and reconciled with supplied totals; contradictory, repeated, and empty non-terminal pages rejected; rate-limit metadata preserved per boundary |
