...
| live smoke harness | ignored by default; clean-commit build without credential variables; read-only sealed executable bound to commit and SHA-256 digest; authenticated phase invokes no Cargo/build tooling; exact read-only opt-in; fixed official origin; private regular token-file input; single-allocation bounded token read and guarded cleanup; Unix symlink, permission, and opened-file identity checks; typed GET-only catalog probes; static diagnostics without token paths, bodies, or IDs; destructive mode rejected |
