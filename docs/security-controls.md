...
| release publishing | readiness and the complete release gate require one clean unchanged `HEAD` at entry and exit; signed annotated tag must verify; publisher captures and revalidates the clean commit, tag target, and signature before every locked crate publication; no normal-path bypass flags |
