...
| optional Serde boundary | default graph exclusion; no Serde or serde_json `std`; 1 MiB request and 8 MiB response policies; duplicate-rejecting JSON admission with a shared 65,536-node budget, bounded depth/containers/strings, and volatile-clearing storage for every parsed string value; source-locked checked decoding for every active operation; typed actions, resource identities, pagination, special results, and provider errors |
