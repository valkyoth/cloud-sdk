use super::{OwnerChangeError as Error, OwnerChangePermit, OwnerChangeResponse};
use crate::{
...
pub struct OwnerChangeBuffers<'a> {
...
/// Single-attempt blocking ownership mutation runner over an explicitly trusted adapter.
pub struct OwnerChangeClient<'a, T: ?Sized>(DiscoveryClient<'a, T>);
impl<T: ?Sized> core::fmt::Debug for OwnerChangeClient<'_, T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.write_str("OwnerChangeClient([redacted])")
    }
}
impl<'a, T: BoundTransport + BoundUserAgent + ?Sized> OwnerChangeClient<'a, T> {
    ...
    /// Consumes explicit consent, sends once, and checks the source acknowledgement.
    ...
    ///
    /// ```compile_fail
    /// use cloud_sdk_cratesio::ownership::*;
    /// use cloud_sdk::transport::{BoundTransport, BoundUserAgent};
    /// fn replay<T: BoundTransport + BoundUserAgent>(c: &OwnerChangeClient<'_, T>, p: OwnerChangePermit<'_>, a: OwnerChangeBuffers<'_>, b: OwnerChangeBuffers<'_>) {
    ...
    /// use cloud_sdk_cratesio::ownership::*;
    /// use cloud_sdk::transport::{BoundTransport, BoundUserAgent};
    /// fn unconfirmed<T: BoundTransport + BoundUserAgent>(c: &OwnerChangeClient<'_, T>, r: OwnerChangeRequest<'_>, b: OwnerChangeBuffers<'_>) {
    ...
    ) -> Result<OwnerChangeResponse, Failure<E>> {
