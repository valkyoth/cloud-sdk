...
use cloud_sdk::{
    Method,
    transport::{
        AsyncRawUploadExecutor, AsyncStreamSource, AuthorizedUpload, BoundUserAgent,
        LocalAsyncStreamSource, LocalRawUploadExecutor, MediaType, RequestHeader, RequestHeaders,
...
macro_rules! execute {
    ($name:ident, $source:path, $framed:ident, $execute:ident $(, $send:ident)?) => {
        ...
        pub fn $name<'s, S: $source $(+ $send)? + 's>(
        ...
        ) -> impl Future<Output = Result<PublishResponse, Failure<T::Error>>> $(+ $send)? + 's {
            ...
            async move {
                ...
                let upload = AuthorizedUpload { expected: material.endpoint(), authorization,
                    source: &mut source, policy: permit.request.policy, scratch: scratch.as_mut_slice() };
...
impl<T: AsyncRawUploadExecutor + BoundUserAgent + ?Sized> PublishClient<'_, T> {
    execute!(
...
impl<T: LocalRawUploadExecutor + BoundUserAgent + ?Sized> PublishClient<'_, T> {
    execute!(
