use super::*;
use crate::{
    accounts::tests::{FIXTURES, requests},
...
impl Executor {
    fn wire(&self) -> &'static [u8] {
        if self.status >= 400 {
            br#"{"errors":[{"detail":"private test error"}]}"#
        } else {
            FIXTURES.get(self.index).fixture("fixture").as_bytes()
        }
    }
...
#[test]
fn all_accounts_operations_run_with_blocking_local_and_send_parity() {
    ...
    for (index, request) in requests().into_iter().enumerate() {
        ...
        let client = AccountClient::production(&executor, identity(), 65_536).fixture("client");
        let local_client = AccountClient::production(&local, identity(), 65_536).fixture("client");
...
#[test]
fn accounts_shares_rate_gate_and_cancellation_cleanup_with_discovery() {
    let _serial = TEST_GATE_LOCK.lock().fixture("gate");
    reset_test_gate();
    ...
    assert!(matches!(
        ...
        Err(AccountExecutionError::Schedule(ScheduleError::Wait(_)))
    ));
    assert_eq!(executor.calls.load(Ordering::SeqCst), 1);
}

#[test]
fn all_account_modes_reject_bad_wire_and_enforce_bounded_delays() {
    let _serial = TEST_GATE_LOCK.lock().fixture("gate");
    for (index, request) in requests().into_iter().enumerate() {
        for (status, media, retry) in [
            ...
            let client = AccountClient::production(&executor, identity(), 65_536).fixture("client");
            let local_client =
                AccountClient::production(&local, identity(), 65_536).fixture("local");
            for mode in 0..3 {
                reset_test_gate();
                let mut body = [0xa5; 65_536];
                let mut headers = [0xa5; 512];
                ...
                assert!(!alloc::format!("{result:?}").contains("private test error"));
                if status == 404 {
                    assert!(
                        matches!(result, Err(AccountExecutionError::Wire(crate::wire::CratesIoWireError::Provider(error))) if error.status() == StatusCode::new(404).fixture("status"))
                    );
                }
                if retry.is_some() {
                    assert!(matches!(
                        result,
                        Err(AccountExecutionError::Schedule(ScheduleError::Overflow))
                    ));
                }
