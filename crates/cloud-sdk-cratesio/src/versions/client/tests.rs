...
use crate::{
    ...
    versions::tests::{FIXTURES, requests},
...
#[test]
fn all_versions_operations_run_with_blocking_local_and_send_parity() {
    ...
    for (index, request) in requests().into_iter().enumerate() {
        ...
        let client = VersionClient::production(&executor, identity(), 65_536).fixture("client");
        let local_client = VersionClient::production(&local, identity(), 65_536).fixture("client");
...
#[test]
fn versions_shares_rate_gate_and_cancellation_cleanup_with_discovery() {
    let _serial = TEST_GATE_LOCK.lock().fixture("gate");
    reset_test_gate();
    ...
    assert!(matches!(
        ...
        Err(VersionExecutionError::Schedule(ScheduleError::Wait(_)))
    ));
    assert_eq!(executor.calls.load(Ordering::SeqCst), 1);
}

#[test]
fn all_version_modes_reject_bad_wire_and_enforce_bounded_delays() {
    let _serial = TEST_GATE_LOCK.lock().fixture("gate");
    for (index, request) in requests().into_iter().enumerate() {
        for (status, media, retry) in [
            ...
            let client = VersionClient::production(&executor, identity(), 65_536).fixture("client");
            let local_client =
                VersionClient::production(&local, identity(), 65_536).fixture("local");
            for mode in 0..3 {
                reset_test_gate();
                let mut body = [0xa5; 65_536];
                let mut headers = [0xa5; 512];
                ...
                if retry.is_some() {
                    assert!(matches!(
                        result,
                        Err(VersionExecutionError::Schedule(ScheduleError::Overflow))
                    ));
                }
