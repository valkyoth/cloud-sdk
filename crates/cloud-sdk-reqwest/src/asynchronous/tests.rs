...
fn build_loopback(endpoint: &str) -> Option<AsyncClient> {
    ...
    let user_agent = UserAgent::new("cloud-sdk-test/0.17").ok()?;
...
async fn async_client_sends_exact_headers_target_and_body_once() {
    let server = spawn(
    ...
    if let Ok(recorded) = recorded {
        ...
        assert!(wire.contains("user-agent: cloud-sdk-test/0.17\r\n"));
...
#[tokio::test(flavor = "current_thread")]
async fn async_redirect_is_not_followed_and_oversized_body_is_rejected() {
    let redirect = spawn(
        "302 Found",
...
#[tokio::test(flavor = "current_thread")]
async fn missing_content_type_fails_before_network_access() {
...
#[tokio::test(flavor = "current_thread")]
async fn internal_timeout_is_payload_free_and_clears_output() {
    ...
    let user_agent = UserAgent::new("cloud-sdk-test/0.17");
