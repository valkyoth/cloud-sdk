...
macro_rules! shared_permit {
    ($name:ident, $inner:ident, $description:literal) => {
        #[doc = $description]
        pub struct $name<'state, 'storage, 'fingerprint, 'request, R>
        where
        ...
        where
            R: RobotClientMutationOperation,
        {
            /// Binds caller-owned atomic state to one exact request and plan.
            pub fn new(
            ...
            /// Atomically rearms after a proven-not-sent result.
        ...
        impl<R: RobotClientMutationOperation> Clone for $name<'_, '_, '_, '_, R> {
            fn clone(&self) -> Self {
                Self {
...
shared_permit!(
    RobotMutationSharedPermit,
    SharedMutationPermit,
    "Shared authority for one exact generic Robot mutation."
);
shared_permit!(
    RobotMutationSharedDestructivePermit,
    SharedDestructivePermit,
    "Shared authority for one exact destructive Robot mutation."
);
