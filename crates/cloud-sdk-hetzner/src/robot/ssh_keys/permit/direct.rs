...
macro_rules! direct_permit {
    ($name:ident, $inner:ident, $description:literal) => {
        #[doc = $description]
        pub struct $name<'storage, 'fingerprint, 'request, R: RobotSshKeyPermitRequest> {
            inner: cloud_sdk::operation::$inner<'storage, 'fingerprint>,
            binding: super::SshKeyBinding<'request, R>,
        }

        impl<'storage, 'fingerprint, 'request, R: RobotSshKeyPermitRequest>
        ...
        impl<R: RobotSshKeyPermitRequest> core::fmt::Debug for $name<'_, '_, '_, R> {
...
direct_permit!(
    RobotSshKeyMutationPermit,
    MutationPermit,
    "Direct request-bound authority for one Robot ssh_keys mutation."
);
direct_permit!(
    RobotSshKeyDestructivePermit,
    DestructivePermit,
    "Direct request-bound authority for one destructive Robot ssh_keys mutation."
);
