...
macro_rules! shared_permit {
    ($name:ident, $inner:ident, $description:literal) => {
        #[doc = $description]
        pub struct $name<'state, 'storage, 'fingerprint, 'request, R: RobotFailoverPermitRequest> {
            inner: cloud_sdk::operation::$inner<'state, 'storage, 'fingerprint>,
            binding: super::FailoverBinding<'request, R>,
        }

        impl<'state, 'storage, 'fingerprint, 'request, R: RobotFailoverPermitRequest>
        ...
        impl<R: RobotFailoverPermitRequest> Clone for $name<'_, '_, '_, '_, R> {
            fn clone(&self) -> Self {
                Self {
...
shared_permit!(
    RobotFailoverSharedMutationPermit,
    SharedMutationPermit,
    "Shared request-bound authority for one Robot failover mutation."
);
shared_permit!(
    RobotFailoverSharedDestructivePermit,
    SharedDestructivePermit,
    "Shared request-bound authority for one destructive Robot failover mutation."
);
