...
#[test]
fn every_source_locked_failover_failure_is_operation_bound() {
    let cases = [
        (
            Case::List,
            404,
    ...
    for (operation, status, code, expected) in cases {
        let failure = decode(operation, status, code)
            .unwrap_or_else(|_| unreachable!("source-locked failover failure was rejected"));
        let RobotFailure::Provider(provider) = failure else {
            unreachable!("failover provider failure changed category")
        };
...
fn cross_operation_failure_widening_fails_closed() {
    assert_eq!(
        decode(Case::List, 409, "FAILOVER_LOCKED").err(),
        Some(RobotDecodeError::UnsupportedStatus)
    );
    assert_eq!(
        decode(Case::Delete, 409, "FAILOVER_ALREADY_ROUTED").err(),
        Some(RobotDecodeError::UnknownCode)
    );
    assert_eq!(
        decode(Case::Reroute, 500, "RESET_FAILED").err(),
        Some(RobotDecodeError::UnknownCode)
    );
}

#[test]
fn invalid_input_is_admitted_only_for_reroute() {
    let body = br#"{"error":{"status":400,"code":"INVALID_INPUT","message":"redacted","missing":null,"invalid":["active_server_ip"]}}"#;
    assert!(matches!(
        decode_body(Case::Reroute, 400, body),
...
fn dispatch(
    ...
    match operation {
        Case::List => RobotFailoverListRequest::new().decode_failure(response, workspace),
        Case::Get => RobotFailoverGetRequest::new(ip()).decode_failure(response, workspace),
        Case::Reroute => RobotFailoverRerouteRequest::new(ip(), destination())
            .unwrap_or_else(|_| unreachable!("reroute fixture failed"))
            .decode_failure(response, workspace),
        Case::Delete => {
            RobotFailoverDeleteRouteRequest::new(ip()).decode_failure(response, workspace)
        }
    }
}
...
fn ip() -> crate::robot::RobotIpAddress {
    crate::robot::RobotIpAddress::new("192.0.2.50")
        .unwrap_or_else(|_| unreachable!("route fixture failed"))
}

fn destination() -> crate::robot::RobotIpAddress {
    crate::robot::RobotIpAddress::new("192.0.2.11")
        .unwrap_or_else(|_| unreachable!("destination fixture failed"))
}
