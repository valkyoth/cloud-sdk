...
use super::{RobotIpPermitAttempt, RobotIpPermitRequest, RobotIpPlanSubject};

macro_rules! shared_permit {
    ($name:ident, $inner:ident, $description:literal) => {
        #[doc = $description]
        pub struct $name<'state, 'storage, 'fingerprint, 'request, R: RobotIpPermitRequest> {
            inner: cloud_sdk::operation::$inner<'state, 'storage, 'fingerprint>,
            binding: super::IpBinding<'request, R>,
        }

        impl<'state, 'storage, 'fingerprint, 'request, R: RobotIpPermitRequest>
            $name<'state, 'storage, 'fingerprint, 'request, R>
        {
            ...
            pub fn new(
                ...
                subject: RobotIpPlanSubject<'storage, 'fingerprint, 'request, R>,
                now: PermitTimestamp,
            ...
            pub fn begin(
                ...
                RobotIpPermitAttempt<'_, 'storage, 'fingerprint, 'request, R>,
                ExecutionPermitError,
            > {
                Ok(RobotIpPermitAttempt {
            ...
            pub fn begin_for(
                &self,
                candidate: RobotIpPlanSubject<'_, '_, '_, R>,
                now: PermitTimestamp,
            ) -> Result<
                RobotIpPermitAttempt<'_, 'storage, 'fingerprint, 'request, R>,
                ExecutionPermitError,
        ...
        impl<R: RobotIpPermitRequest> Clone for $name<'_, '_, '_, '_, R> {
            fn clone(&self) -> Self {
                Self {
...
shared_permit!(
    RobotIpSharedMutationPermit,
    SharedMutationPermit,
    "Shared request-bound authority for one Robot IP mutation."
);
shared_permit!(
    RobotIpSharedDestructivePermit,
    SharedDestructivePermit,
    "Shared request-bound authority for one destructive Robot IP mutation."
);
