...
macro_rules! direct_permit {
    ($name:ident, $inner:ident, $description:literal) => {
        #[doc = $description]
        pub struct $name<'storage, 'fingerprint, 'request, R: RobotIpPermitRequest> {
            inner: cloud_sdk::operation::$inner<'storage, 'fingerprint>,
            binding: super::IpBinding<'request, R>,
        }

        impl<'storage, 'fingerprint, 'request, R: RobotIpPermitRequest>
        ...
        impl<R: RobotIpPermitRequest> core::fmt::Debug for $name<'_, '_, '_, R> {
...
direct_permit!(
    RobotIpMutationPermit,
    MutationPermit,
    "Direct request-bound authority for one Robot IP mutation."
);
direct_permit!(
    RobotIpDestructivePermit,
    DestructivePermit,
    "Direct request-bound authority for one destructive Robot IP mutation."
);
