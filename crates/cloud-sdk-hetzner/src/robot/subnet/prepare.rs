...
use crate::robot::{
    RobotForm, RobotFormField, RobotIpAddress, RobotMacAddress, RobotSubnetAddress,
};
...
macro_rules! prepare_operation {
    ($type:ty, $kind:ident, $($field:ident),+ $(,)?) => {
        impl PrepareOperation for $type {
            type Error = RobotSubnetRequestError;
            fn prepare<'storage>(
                &self,
...
impl PrepareOperation for RobotSubnetListRequest {
    type Error = RobotSubnetRequestError;
    fn prepare<'storage>(
        &self,
        storage: PreparationStorage<'storage>,
...
prepare_operation!(RobotSubnetGetRequest, Get, address);
impl PrepareOperation for RobotSubnetUpdateRequest {
    type Error = RobotSubnetRequestError;
    fn prepare<'storage>(
        &self,
        storage: PreparationStorage<'storage>,
...
prepare_operation!(RobotSubnetMacSetRequest, SetMac, address, mac);
prepare_operation!(RobotSubnetMacDeleteRequest, DeleteMac, address);

fn prepare<'storage>(
    kind: Kind<'_>,
    storage: PreparationStorage<'storage>,
) -> Result<PreparedRequest<'storage>, RobotSubnetRequestError> {
    ...
    let endpoint = admitted!(
        official_robot_endpoint_policy().map_err(RobotSubnetRequestError::InvalidEndpoint)
    );
    let endpoint_identity = admitted!(
        official_robot_endpoint_identity().map_err(RobotSubnetRequestError::InvalidEndpoint)
    );
    ...
    let response = admitted!(
        ...
        .map_err(RobotSubnetRequestError::InvalidResponsePolicy)
    );
    let content =
        admitted!(HeaderName::new("content-type").map_err(RobotSubnetRequestError::InvalidHeaders));
    let raw = admitted!(
    ...
    let body_len = match kind {
        ...
        Kind::SetMac(_, mac) => admitted!(write_mac_form(mac, body_storage)),
        _ => 0,
    };
    let headers = admitted!(
        ...
        .map_err(RobotSubnetRequestError::InvalidHeaders)
    );
    let operation_id =
        admitted!(OperationId::new(id(kind)).map_err(RobotSubnetRequestError::InvalidOperationId));
    ...
    let Some(target_text) = target_storage
        ...
        unreachable!("validated Robot subnet target lost UTF-8")
    };
    let Ok(target) = RequestTarget::new(target_text) else {
        unreachable!("validated Robot subnet target became invalid")
    };
...
fn write_target(kind: Kind<'_>, output: &mut [u8]) -> Result<usize, RobotSubnetRequestError> {
    let mut len = 0;
    write_str(output, &mut len, "/subnet", RobotSubnetRequestError::Path)?;
    match kind {
        Kind::List(Some(server)) => server.with_text(|text| {
            write_str(output, &mut len, "?", RobotSubnetRequestError::Path)?;
            let mut first = true;
            write_query_pair(
                output,
                &mut len,
                ...
                "server_ip",
                text,
                RobotSubnetRequestError::Path,
            )
        })?,
        Kind::List(None) => {}
        _ => {
            write_str(output, &mut len, "/", RobotSubnetRequestError::Path)?;
            address(kind).with_text(|text| {
                write_str(output, &mut len, text, RobotSubnetRequestError::Path)
            })?;
            if matches!(
                kind,
                Kind::GetMac(_) | Kind::SetMac(_, _) | Kind::DeleteMac(_)
            ) {
                write_str(output, &mut len, "/mac", RobotSubnetRequestError::Path)?;
            }
        }
    }
    Ok(len)
...
fn write_update_form(
    ...
    let placeholder = RobotFormField::sensitive("traffic_warnings", warnings)
        .map_err(RobotSubnetRequestError::Form)?;
    let mut fields = [placeholder; 4];
    let mut len = 0;
    macro_rules! push {
        ($name:literal, $value:expr) => {{
            let field =
                RobotFormField::sensitive($name, $value).map_err(RobotSubnetRequestError::Form)?;
            *fields.get_mut(len).ok_or(RobotSubnetRequestError::Path)? = field;
            len = len.checked_add(1).ok_or(RobotSubnetRequestError::Path)?;
        }};
    }
    ...
    let selected = fields.get(..len).ok_or(RobotSubnetRequestError::Path)?;
...
fn write_mac_form(
    ...
    mac.try_with_text(|text| {
        let field =
            RobotFormField::sensitive("mac", text).map_err(RobotSubnetRequestError::Form)?;
        RobotForm::new(core::slice::from_ref(&field))
            .and_then(|form| form.write_prepared(output))
            .map_err(RobotSubnetRequestError::Form)
    })
    .map_err(|_| RobotSubnetRequestError::Path)?
}
...
fn address(kind: Kind<'_>) -> &RobotSubnetAddress {
    match kind {
        Kind::Get(value)
        | Kind::Update(value, _)
        | Kind::GetMac(value)
...
const fn id(kind: Kind<'_>) -> &'static str {
    match kind {
        Kind::List(_) => "robot_list_subnets",
        Kind::Get(_) => "robot_get_subnet",
        Kind::Update(_, _) => "robot_update_subnet",
        Kind::GetMac(_) => "robot_get_subnet_mac",
        Kind::SetMac(_, _) => "robot_set_subnet_mac",
        Kind::DeleteMac(_) => "robot_delete_subnet_mac",
    }
}

fn metadata(kind: Kind<'_>) -> Result<OperationMetadata, RobotSubnetRequestError> {
    ...
    .map_err(RobotSubnetRequestError::InvalidMetadata)
}
