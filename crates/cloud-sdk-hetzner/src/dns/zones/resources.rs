...
use crate::cloud::shared::{encode_query, write_static_path};
    ...
    pub fn write_query(self, output: &mut [u8]) -> Result<usize, ZoneRequestError> {
        Ok(encode_query(output, |writer, first| {
            ...
            if let Some(mode) = self.mode {
                writer.query_pair(first, "mode", mode.as_api_str())?;
            }
            ...
            if let Some((field, direction)) = self.sort {
                writer.query_pair(first, "sort", zone_sort_value(field, direction))?;
            }
            Ok(())
        })?)
    }
