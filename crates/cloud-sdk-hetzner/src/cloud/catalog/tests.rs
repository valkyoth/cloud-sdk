...
fn catalog_list_query_writes_pagination_and_sorting() {
    ...
    let mut output = [0u8; 64];
    let (Ok(page), Ok(per_page), Ok(sort_key)) = (page, per_page, sort_key) else {
        unreachable!("security fixture construction failed");
    };
...
fn catalog_sortable_list_endpoints_write_sort_query_from_api_matrix() {
    ...
    let sort_key = SortKey::new("name");
    let Ok(sort_key) = sort_key else {
        unreachable!("security fixture construction failed");
    };
    for endpoint in endpoints {
        let mut output = [0u8; 64];
        let request =
            CatalogListRequest::new(endpoint).with_sort(Sort::new(sort_key, SortDirection::Desc));
        let Ok(request) = request else {
            unreachable!("security fixture construction failed");
        };
        assert_eq!(
