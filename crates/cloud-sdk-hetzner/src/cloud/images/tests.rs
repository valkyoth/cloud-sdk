use super::{
    ImageActionEndpoint, ImageArchitecture, ImageEndpoint, ImageId, ImageListRequest,
...
fn server_adjacent_image_list_query_writes_filters_pagination_and_sorting() {
    ...
    assert_eq!(
        query,
        Some(
            "architecture=arm&bound_to=42&include_deprecated=false&page=2&per_page=25&sort=created%3Adesc&status=available&type=snapshot"
        )
    );
}
