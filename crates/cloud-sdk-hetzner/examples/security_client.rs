//! Complete blocking Security client workflow against a deterministic transport.
...
use cloud_sdk_hetzner::association::operations::ListCertificates;
...
const RESPONSE: &[u8] = br#"{"certificates":[],"meta":{"pagination":{"page":1,"per_page":25,"previous_page":null,"next_page":null,"last_page":1,"total_entries":0}}}"#;

fn main() -> Result<(), Box<dyn core::error::Error>> {
    let operation = AssociatedOperation::<ListCertificates, _, _>::query(
    ...
    let checked = client.list_certificates_blocking(&operation, lease)?;
    assert!(matches!(
        checked.success(),
        HetznerSuccess::SecurityResources { .. }
    ));
