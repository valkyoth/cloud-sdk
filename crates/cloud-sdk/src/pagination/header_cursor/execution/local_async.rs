...
{
    /// Executes the next local async request with retained context.
    #[allow(clippy::too_many_arguments)]
    pub async fn execute_local_async<'response, 'next, T>(
        &self,
        transport: &T,
        response_storage: &'response mut [u8],
        response_header_storage: &'response mut [u8],
...
#[allow(clippy::too_many_arguments)]
async fn execute_local_async<'response, 'cursor, 'endpoint, 'session, 'request, 'policy, T>(
